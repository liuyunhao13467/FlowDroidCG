...
//		seeAllMethods(tmpApk);
	...
	public static void fillOneEdge(ResultSet rsCall ,Set<MyEdge> edges) throws SQLException{
		...
		tmpEdge = new MyEdge();
		tmpEdge.setSrcMethodIDInApk(rsCall.getInt("caller_id"));
		tmpEdge.setTgtMethodIDInApk(rsCall.getInt("callee_id"));
		edges.add(tmpEdge);
	}
	...
	public static Map<Integer, List<Integer>>  getOutEdges(Apk apk){
		...
		while(edgeIt.hasNext()){
			tmpEdge = edgeIt.next();
			if(outEdges.containsKey(tmpEdge.getSrcMethodIDInApk()) && outEdges.get(tmpEdge.getSrcMethodIDInApk()) != null){
				outEdges.get(tmpEdge.getSrcMethodIDInApk()).add(tmpEdge.getTgtMethodIDInApk());
			}else{
				List<Integer> calleeNodes = new ArrayList<Integer>();
				calleeNodes.add(tmpEdge.getTgtMethodIDInApk());
				outEdges.put(tmpEdge.getSrcMethodIDInApk(), calleeNodes);
			}
		}
